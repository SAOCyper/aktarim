...
function sendTileData(data: Buffer, res: any) {
    let tileBuffer: Buffer = data;
    // Decompress GZIP tiles (MBTiles often stores gzip-compressed images)
    if (tileBuffer && tileBuffer.length > 2 && tileBuffer[0] === 0x1f && tileBuffer[1] === 0x8b) {
        try {
            tileBuffer = zlib.gunzipSync(tileBuffer);
        } catch {
            // Can't decompress — pass raw with Content-Encoding header
    ...
    } else if (tileBuffer && tileBuffer.length > 3 && tileBuffer[0] === 0x52 && tileBuffer[1] === 0x49) {
        res.set('Content-Type', 'image/webp'); // RIFF/WEBP
    } else {
        res.set('Content-Type', 'image/png');
    }
...
export class SocBackendContribution implements BackendApplicationContribution {
    ...
    configure(app: any): void {
        ...
        const serveTile = (dataset: string, z: string, x: string, y: string, res: any) => {
            const state = readMbtilesState();
            const allFiles = listMbtilesForDataset(dataset, state);
            const files = allFiles.filter(f => f.enabled);
            console.log(`[serveTile] dataset=${dataset} z=${z} x=${x} y=${y} MBTILES_DIR=${MBTILES_DIR}`);
            console.log(`[serveTile] all files:`, JSON.stringify(allFiles.map(f => ({ filename: f.filename, enabled: f.enabled, path: f.path }))));
            if (files.length === 0) {
                console.warn(`[serveTile] No enabled files for dataset=${dataset}. State:`, JSON.stringify(state));
